...
void GameShaders::Bind()
{
	glUseProgram(program); //installs the program object specified by program as part of rendering state
}

void GameShaders::Update(const Transform& transform, const MainCamera& camera)
{
	glm::mat4 mvp = camera.FindViewProjection() * transform.GetModel();
